            ...
            public:
                ...
                void publishKeyframes( std::vector<FrameHessian*> &frames, bool final, CalibHessian* HCalib) override {
                    ...
                    for (FrameHessian* fh : frames){
                        ...
                        int p0 = 0; //starting position of each point
                        for (PointHessian* ph : fh->pointHessians){
                            ...
                            uint8_t *castPtr_x = reinterpret_cast<uint8_t*>(&x);
                            uint8_t *castPtr_y = reinterpret_cast<uint8_t*>(&y);
                            uint8_t *castPtr_z = reinterpret_cast<uint8_t*>(&z);

                            for (int offset=0; offset<4; offset++){
                                pc_msg.data[p0+offset] = castPtr_x[offset];
                                pc_msg.data[p0+4+offset] = castPtr_y[offset];
                                pc_msg.data[p0+8+offset] = castPtr_z[offset];
                            }
